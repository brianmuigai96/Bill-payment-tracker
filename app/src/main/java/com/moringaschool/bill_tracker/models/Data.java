
package com.moringaschool.bill_tracker;


import com.google.gson.annotations.Expose;
import com.google.gson.annotations.SerializedName;


public class Data {

    @SerializedName("JPY")
    @Expose
    private Double jpy;
    @SerializedName("CNY")
    @Expose
    private Double cny;
    @SerializedName("CHF")
    @Expose
    private Double chf;
    @SerializedName("CAD")
    @Expose
    private Double cad;
    @SerializedName("MXN")
    @Expose
    private Double mxn;
    @SerializedName("INR")
    @Expose
    private Double inr;
    @SerializedName("BRL")
    @Expose
    private Double brl;
    @SerializedName("RUB")
    @Expose
    private Double rub;
    @SerializedName("KRW")
    @Expose
    private Double krw;
    @SerializedName("IDR")
    @Expose
    private Double idr;
    @SerializedName("TRY")
    @Expose
    private Double _try;
    @SerializedName("SAR")
    @Expose
    private Double sar;
    @SerializedName("SEK")
    @Expose
    private Double sek;
    @SerializedName("NGN")
    @Expose
    private Double ngn;
    @SerializedName("PLN")
    @Expose
    private Double pln;
    @SerializedName("ARS")
    @Expose
    private Double ars;
    @SerializedName("NOK")
    @Expose
    private Double nok;
    @SerializedName("TWD")
    @Expose
    private Double twd;
    @SerializedName("IRR")
    @Expose
    private Double irr;
    @SerializedName("AED")
    @Expose
    private Double aed;
    @SerializedName("COP")
    @Expose
    private Double cop;
    @SerializedName("THB")
    @Expose
    private Double thb;
    @SerializedName("ZAR")
    @Expose
    private Double zar;
    @SerializedName("DKK")
    @Expose
    private Double dkk;
    @SerializedName("MYR")
    @Expose
    private Double myr;
    @SerializedName("SGD")
    @Expose
    private Double sgd;
    @SerializedName("ILS")
    @Expose
    private Double ils;
    @SerializedName("HKD")
    @Expose
    private Double hkd;
    @SerializedName("EGP")
    @Expose
    private Double egp;
    @SerializedName("PHP")
    @Expose
    private Double php;
    @SerializedName("CLP")
    @Expose
    private Double clp;
    @SerializedName("PKR")
    @Expose
    private Double pkr;
    @SerializedName("IQD")
    @Expose
    private Double iqd;
    @SerializedName("DZD")
    @Expose
    private Double dzd;
    @SerializedName("KZT")
    @Expose
    private Double kzt;
    @SerializedName("QAR")
    @Expose
    private Double qar;
    @SerializedName("CZK")
    @Expose
    private Double czk;
    @SerializedName("PEN")
    @Expose
    private Double pen;
    @SerializedName("RON")
    @Expose
    private Double ron;
    @SerializedName("VND")
    @Expose
    private Double vnd;
    @SerializedName("BDT")
    @Expose
    private Double bdt;
    @SerializedName("HUF")
    @Expose
    private Double huf;
    @SerializedName("UAH")
    @Expose
    private Double uah;
    @SerializedName("AOA")
    @Expose
    private Double aoa;
    @SerializedName("MAD")
    @Expose
    private Double mad;
    @SerializedName("OMR")
    @Expose
    private Double omr;
    @SerializedName("CUC")
    @Expose
    private Double cuc;
    @SerializedName("BYR")
    @Expose
    private Double byr;
    @SerializedName("AZN")
    @Expose
    private Double azn;
    @SerializedName("LKR")
    @Expose
    private Double lkr;
    @SerializedName("SDG")
    @Expose
    private Double sdg;
    @SerializedName("SYP")
    @Expose
    private Double syp;
    @SerializedName("MMK")
    @Expose
    private Double mmk;
    @SerializedName("DOP")
    @Expose
    private Double dop;
    @SerializedName("UZS")
    @Expose
    private Double uzs;
    @SerializedName("KES")
    @Expose
    private Double kes;
    @SerializedName("GTQ")
    @Expose
    private Double gtq;
    @SerializedName("URY")
    @Expose
    private Double ury;
    @SerializedName("HRV")
    @Expose
    private Double hrv;
    @SerializedName("MOP")
    @Expose
    private Double mop;
    @SerializedName("ETB")
    @Expose
    private Double etb;
    @SerializedName("CRC")
    @Expose
    private Double crc;
    @SerializedName("TZS")
    @Expose
    private Double tzs;
    @SerializedName("TMT")
    @Expose
    private Double tmt;
    @SerializedName("TND")
    @Expose
    private Double tnd;
    @SerializedName("PAB")
    @Expose
    private Double pab;
    @SerializedName("LBP")
    @Expose
    private Double lbp;
    @SerializedName("RSD")
    @Expose
    private Double rsd;
    @SerializedName("LYD")
    @Expose
    private Double lyd;
    @SerializedName("GHS")
    @Expose
    private Double ghs;
    @SerializedName("YER")
    @Expose
    private Double yer;
    @SerializedName("BOB")
    @Expose
    private Double bob;
    @SerializedName("BHD")
    @Expose
    private Double bhd;
    @SerializedName("CDF")
    @Expose
    private Double cdf;
    @SerializedName("PYG")
    @Expose
    private Double pyg;
    @SerializedName("UGX")
    @Expose
    private Double ugx;
    @SerializedName("SVC")
    @Expose
    private Double svc;
    @SerializedName("TTD")
    @Expose
    private Double ttd;
    @SerializedName("AFN")
    @Expose
    private Double afn;
    @SerializedName("NPR")
    @Expose
    private Double npr;
    @SerializedName("HNL")
    @Expose
    private Double hnl;
    @SerializedName("BIH")
    @Expose
    private Double bih;
    @SerializedName("BND")
    @Expose
    private Double bnd;
    @SerializedName("ISK")
    @Expose
    private Double isk;
    @SerializedName("KHR")
    @Expose
    private Double khr;
    @SerializedName("GEL")
    @Expose
    private Double gel;
    @SerializedName("MZN")
    @Expose
    private Double mzn;
    @SerializedName("BWP")
    @Expose
    private Double bwp;
    @SerializedName("PGK")
    @Expose
    private Double pgk;
    @SerializedName("JMD")
    @Expose
    private Double jmd;
    @SerializedName("XAF")
    @Expose
    private Double xaf;
    @SerializedName("NAD")
    @Expose
    private Double nad;
    @SerializedName("ALL")
    @Expose
    private Double all;
    @SerializedName("SSP")
    @Expose
    private Double ssp;
    @SerializedName("MUR")
    @Expose
    private Double mur;
    @SerializedName("MNT")
    @Expose
    private Double mnt;
    @SerializedName("NIO")
    @Expose
    private Double nio;
    @SerializedName("LAK")
    @Expose
    private Double lak;
    @SerializedName("MKD")
    @Expose
    private Double mkd;
    @SerializedName("AMD")
    @Expose
    private Double amd;
    @SerializedName("MGA")
    @Expose
    private Double mga;
    @SerializedName("XPF")
    @Expose
    private Double xpf;
    @SerializedName("TJS")
    @Expose
    private Double tjs;
    @SerializedName("HTG")
    @Expose
    private Double htg;
    @SerializedName("BSD")
    @Expose
    private Double bsd;
    @SerializedName("MDL")
    @Expose
    private Double mdl;
    @SerializedName("RWF")
    @Expose
    private Double rwf;
    @SerializedName("KGS")
    @Expose
    private Double kgs;
    @SerializedName("GNF")
    @Expose
    private Double gnf;
    @SerializedName("SRD")
    @Expose
    private Double srd;
    @SerializedName("SLL")
    @Expose
    private Double sll;
    @SerializedName("XOF")
    @Expose
    private Double xof;
    @SerializedName("MWK")
    @Expose
    private Double mwk;
    @SerializedName("FJD")
    @Expose
    private Double fjd;
    @SerializedName("ERN")
    @Expose
    private Double ern;
    @SerializedName("SZL")
    @Expose
    private Double szl;
    @SerializedName("GYD")
    @Expose
    private Double gyd;
    @SerializedName("BIF")
    @Expose
    private Double bif;
    @SerializedName("KYD")
    @Expose
    private Double kyd;
    @SerializedName("MVR")
    @Expose
    private Double mvr;
    @SerializedName("LSL")
    @Expose
    private Double lsl;
    @SerializedName("LRD")
    @Expose
    private Double lrd;
    @SerializedName("CVE")
    @Expose
    private Double cve;
    @SerializedName("DJF")
    @Expose
    private Double djf;
    @SerializedName("SCR")
    @Expose
    private Double scr;
    @SerializedName("SOS")
    @Expose
    private Double sos;
    @SerializedName("GMD")
    @Expose
    private Double gmd;
    @SerializedName("KMF")
    @Expose
    private Double kmf;
    @SerializedName("STD")
    @Expose
    private Double std;
    @SerializedName("BTC")
    @Expose
    private Double btc;
    @SerializedName("XRP")
    @Expose
    private Double xrp;
    @SerializedName("AUD")
    @Expose
    private Double aud;
    @SerializedName("BGN")
    @Expose
    private Double bgn;
    @SerializedName("JOD")
    @Expose
    private Double jod;
    @SerializedName("GBP")
    @Expose
    private Double gbp;
    @SerializedName("ETH")
    @Expose
    private Double eth;
    @SerializedName("EUR")
    @Expose
    private Double eur;
    @SerializedName("LTC")
    @Expose
    private Double ltc;
    @SerializedName("NZD")
    @Expose
    private Double nzd;

    /**
     * No args constructor for use in serialization
     * 
     */
    public Data() {
    }

    /**
     * 
     * @param bih
     * @param mwk
     * @param mga
     * @param egp
     * @param mop
     * @param nad
     * @param ssp
     * @param nio
     * @param pen
     * @param nzd
     * @param tmt
     * @param brl
     * @param fjd
     * @param mxn
     * @param std
     * @param scr
     * @param cdf
     * @param gtq
     * @param clp
     * @param ugx
     * @param hnl
     * @param zar
     * @param tnd
     * @param cuc
     * @param bsd
     * @param sll
     * @param sdg
     * @param iqd
     * @param gmd
     * @param twd
     * @param rsd
     * @param dop
     * @param kmf
     * @param myr
     * @param xof
     * @param gel
     * @param btc
     * @param mad
     * @param cve
     * @param azn
     * @param omr
     * @param pgk
     * @param sek
     * @param kes
     * @param uah
     * @param gnf
     * @param mzn
     * @param ern
     * @param svc
     * @param ars
     * @param qar
     * @param irr
     * @param cny
     * @param thb
     * @param uzs
     * @param xpf
     * @param bdt
     * @param lyd
     * @param php
     * @param ury
     * @param rub
     * @param pyg
     * @param isk
     * @param jmd
     * @param cop
     * @param mkd
     * @param dzd
     * @param pab
     * @param sgd
     * @param etb
     * @param kgs
     * @param sos
     * @param lak
     * @param eth
     * @param bnd
     * @param xaf
     * @param lrd
     * @param chf
     * @param all
     * @param djf
     * @param tzs
     * @param vnd
     * @param hrv
     * @param ils
     * @param aud
     * @param ghs
     * @param gyd
     * @param bob
     * @param khr
     * @param mdl
     * @param idr
     * @param kyd
     * @param xrp
     * @param amd
     * @param bwp
     * @param lbp
     * @param tjs
     * @param jod
     * @param hkd
     * @param rwf
     * @param aed
     * @param eur
     * @param lsl
     * @param dkk
     * @param cad
     * @param bgn
     * @param mmk
     * @param nok
     * @param syp
     * @param mur
     * @param _try
     * @param ron
     * @param lkr
     * @param ngn
     * @param czk
     * @param crc
     * @param pkr
     * @param htg
     * @param ltc
     * @param bhd
     * @param kzt
     * @param srd
     * @param szl
     * @param sar
     * @param ttd
     * @param yer
     * @param afn
     * @param mvr
     * @param inr
     * @param krw
     * @param npr
     * @param jpy
     * @param aoa
     * @param mnt
     * @param pln
     * @param gbp
     * @param huf
     * @param byr
     * @param bif
     */
    public Data(Double jpy, Double cny, Double chf, Double cad, Double mxn, Double inr, Double brl, Double rub, Double krw, Double idr, Double _try, Double sar, Double sek, Double ngn, Double pln, Double ars, Double nok, Double twd, Double irr, Double aed, Double cop, Double thb, Double zar, Double dkk, Double myr, Double sgd, Double ils, Double hkd, Double egp, Double php, Double clp, Double pkr, Double iqd, Double dzd, Double kzt, Double qar, Double czk, Double pen, Double ron, Double vnd, Double bdt, Double huf, Double uah, Double aoa, Double mad, Double omr, Double cuc, Double byr, Double azn, Double lkr, Double sdg, Double syp, Double mmk, Double dop, Double uzs, Double kes, Double gtq, Double ury, Double hrv, Double mop, Double etb, Double crc, Double tzs, Double tmt, Double tnd, Double pab, Double lbp, Double rsd, Double lyd, Double ghs, Double yer, Double bob, Double bhd, Double cdf, Double pyg, Double ugx, Double svc, Double ttd, Double afn, Double npr, Double hnl, Double bih, Double bnd, Double isk, Double khr, Double gel, Double mzn, Double bwp, Double pgk, Double jmd, Double xaf, Double nad, Double all, Double ssp, Double mur, Double mnt, Double nio, Double lak, Double mkd, Double amd, Double mga, Double xpf, Double tjs, Double htg, Double bsd, Double mdl, Double rwf, Double kgs, Double gnf, Double srd, Double sll, Double xof, Double mwk, Double fjd, Double ern, Double szl, Double gyd, Double bif, Double kyd, Double mvr, Double lsl, Double lrd, Double cve, Double djf, Double scr, Double sos, Double gmd, Double kmf, Double std, Double btc, Double xrp, Double aud, Double bgn, Double jod, Double gbp, Double eth, Double eur, Double ltc, Double nzd) {
        super();
        this.jpy = jpy;
        this.cny = cny;
        this.chf = chf;
        this.cad = cad;
        this.mxn = mxn;
        this.inr = inr;
        this.brl = brl;
        this.rub = rub;
        this.krw = krw;
        this.idr = idr;
        this._try = _try;
        this.sar = sar;
        this.sek = sek;
        this.ngn = ngn;
        this.pln = pln;
        this.ars = ars;
        this.nok = nok;
        this.twd = twd;
        this.irr = irr;
        this.aed = aed;
        this.cop = cop;
        this.thb = thb;
        this.zar = zar;
        this.dkk = dkk;
        this.myr = myr;
        this.sgd = sgd;
        this.ils = ils;
        this.hkd = hkd;
        this.egp = egp;
        this.php = php;
        this.clp = clp;
        this.pkr = pkr;
        this.iqd = iqd;
        this.dzd = dzd;
        this.kzt = kzt;
        this.qar = qar;
        this.czk = czk;
        this.pen = pen;
        this.ron = ron;
        this.vnd = vnd;
        this.bdt = bdt;
        this.huf = huf;
        this.uah = uah;
        this.aoa = aoa;
        this.mad = mad;
        this.omr = omr;
        this.cuc = cuc;
        this.byr = byr;
        this.azn = azn;
        this.lkr = lkr;
        this.sdg = sdg;
        this.syp = syp;
        this.mmk = mmk;
        this.dop = dop;
        this.uzs = uzs;
        this.kes = kes;
        this.gtq = gtq;
        this.ury = ury;
        this.hrv = hrv;
        this.mop = mop;
        this.etb = etb;
        this.crc = crc;
        this.tzs = tzs;
        this.tmt = tmt;
        this.tnd = tnd;
        this.pab = pab;
        this.lbp = lbp;
        this.rsd = rsd;
        this.lyd = lyd;
        this.ghs = ghs;
        this.yer = yer;
        this.bob = bob;
        this.bhd = bhd;
        this.cdf = cdf;
        this.pyg = pyg;
        this.ugx = ugx;
        this.svc = svc;
        this.ttd = ttd;
        this.afn = afn;
        this.npr = npr;
        this.hnl = hnl;
        this.bih = bih;
        this.bnd = bnd;
        this.isk = isk;
        this.khr = khr;
        this.gel = gel;
        this.mzn = mzn;
        this.bwp = bwp;
        this.pgk = pgk;
        this.jmd = jmd;
        this.xaf = xaf;
        this.nad = nad;
        this.all = all;
        this.ssp = ssp;
        this.mur = mur;
        this.mnt = mnt;
        this.nio = nio;
        this.lak = lak;
        this.mkd = mkd;
        this.amd = amd;
        this.mga = mga;
        this.xpf = xpf;
        this.tjs = tjs;
        this.htg = htg;
        this.bsd = bsd;
        this.mdl = mdl;
        this.rwf = rwf;
        this.kgs = kgs;
        this.gnf = gnf;
        this.srd = srd;
        this.sll = sll;
        this.xof = xof;
        this.mwk = mwk;
        this.fjd = fjd;
        this.ern = ern;
        this.szl = szl;
        this.gyd = gyd;
        this.bif = bif;
        this.kyd = kyd;
        this.mvr = mvr;
        this.lsl = lsl;
        this.lrd = lrd;
        this.cve = cve;
        this.djf = djf;
        this.scr = scr;
        this.sos = sos;
        this.gmd = gmd;
        this.kmf = kmf;
        this.std = std;
        this.btc = btc;
        this.xrp = xrp;
        this.aud = aud;
        this.bgn = bgn;
        this.jod = jod;
        this.gbp = gbp;
        this.eth = eth;
        this.eur = eur;
        this.ltc = ltc;
        this.nzd = nzd;
    }

    public Double getJpy() {
        return jpy;
    }

    public void setJpy(Double jpy) {
        this.jpy = jpy;
    }

    public Double getCny() {
        return cny;
    }

    public void setCny(Double cny) {
        this.cny = cny;
    }

    public Double getChf() {
        return chf;
    }

    public void setChf(Double chf) {
        this.chf = chf;
    }

    public Double getCad() {
        return cad;
    }

    public void setCad(Double cad) {
        this.cad = cad;
    }

    public Double getMxn() {
        return mxn;
    }

    public void setMxn(Double mxn) {
        this.mxn = mxn;
    }

    public Double getInr() {
        return inr;
    }

    public void setInr(Double inr) {
        this.inr = inr;
    }

    public Double getBrl() {
        return brl;
    }

    public void setBrl(Double brl) {
        this.brl = brl;
    }

    public Double getRub() {
        return rub;
    }

    public void setRub(Double rub) {
        this.rub = rub;
    }

    public Double getKrw() {
        return krw;
    }

    public void setKrw(Double krw) {
        this.krw = krw;
    }

    public Double getIdr() {
        return idr;
    }

    public void setIdr(Double idr) {
        this.idr = idr;
    }

    public Double getTry() {
        return _try;
    }

    public void setTry(Double _try) {
        this._try = _try;
    }

    public Double getSar() {
        return sar;
    }

    public void setSar(Double sar) {
        this.sar = sar;
    }

    public Double getSek() {
        return sek;
    }

    public void setSek(Double sek) {
        this.sek = sek;
    }

    public Double getNgn() {
        return ngn;
    }

    public void setNgn(Double ngn) {
        this.ngn = ngn;
    }

    public Double getPln() {
        return pln;
    }

    public void setPln(Double pln) {
        this.pln = pln;
    }

    public Double getArs() {
        return ars;
    }

    public void setArs(Double ars) {
        this.ars = ars;
    }

    public Double getNok() {
        return nok;
    }

    public void setNok(Double nok) {
        this.nok = nok;
    }

    public Double getTwd() {
        return twd;
    }

    public void setTwd(Double twd) {
        this.twd = twd;
    }

    public Double getIrr() {
        return irr;
    }

    public void setIrr(Double irr) {
        this.irr = irr;
    }

    public Double getAed() {
        return aed;
    }

    public void setAed(Double aed) {
        this.aed = aed;
    }

    public Double getCop() {
        return cop;
    }

    public void setCop(Double cop) {
        this.cop = cop;
    }

    public Double getThb() {
        return thb;
    }

    public void setThb(Double thb) {
        this.thb = thb;
    }

    public Double getZar() {
        return zar;
    }

    public void setZar(Double zar) {
        this.zar = zar;
    }

    public Double getDkk() {
        return dkk;
    }

    public void setDkk(Double dkk) {
        this.dkk = dkk;
    }

    public Double getMyr() {
        return myr;
    }

    public void setMyr(Double myr) {
        this.myr = myr;
    }

    public Double getSgd() {
        return sgd;
    }

    public void setSgd(Double sgd) {
        this.sgd = sgd;
    }

    public Double getIls() {
        return ils;
    }

    public void setIls(Double ils) {
        this.ils = ils;
    }

    public Double getHkd() {
        return hkd;
    }

    public void setHkd(Double hkd) {
        this.hkd = hkd;
    }

    public Double getEgp() {
        return egp;
    }

    public void setEgp(Double egp) {
        this.egp = egp;
    }

    public Double getPhp() {
        return php;
    }

    public void setPhp(Double php) {
        this.php = php;
    }

    public Double getClp() {
        return clp;
    }

    public void setClp(Double clp) {
        this.clp = clp;
    }

    public Double getPkr() {
        return pkr;
    }

    public void setPkr(Double pkr) {
        this.pkr = pkr;
    }

    public Double getIqd() {
        return iqd;
    }

    public void setIqd(Double iqd) {
        this.iqd = iqd;
    }

    public Double getDzd() {
        return dzd;
    }

    public void setDzd(Double dzd) {
        this.dzd = dzd;
    }

    public Double getKzt() {
        return kzt;
    }

    public void setKzt(Double kzt) {
        this.kzt = kzt;
    }

    public Double getQar() {
        return qar;
    }

    public void setQar(Double qar) {
        this.qar = qar;
    }

    public Double getCzk() {
        return czk;
    }

    public void setCzk(Double czk) {
        this.czk = czk;
    }

    public Double getPen() {
        return pen;
    }

    public void setPen(Double pen) {
        this.pen = pen;
    }

    public Double getRon() {
        return ron;
    }

    public void setRon(Double ron) {
        this.ron = ron;
    }

    public Double getVnd() {
        return vnd;
    }

    public void setVnd(Double vnd) {
        this.vnd = vnd;
    }

    public Double getBdt() {
        return bdt;
    }

    public void setBdt(Double bdt) {
        this.bdt = bdt;
    }

    public Double getHuf() {
        return huf;
    }

    public void setHuf(Double huf) {
        this.huf = huf;
    }

    public Double getUah() {
        return uah;
    }

    public void setUah(Double uah) {
        this.uah = uah;
    }

    public Double getAoa() {
        return aoa;
    }

    public void setAoa(Double aoa) {
        this.aoa = aoa;
    }

    public Double getMad() {
        return mad;
    }

    public void setMad(Double mad) {
        this.mad = mad;
    }

    public Double getOmr() {
        return omr;
    }

    public void setOmr(Double omr) {
        this.omr = omr;
    }

    public Double getCuc() {
        return cuc;
    }

    public void setCuc(Double cuc) {
        this.cuc = cuc;
    }

    public Double getByr() {
        return byr;
    }

    public void setByr(Double byr) {
        this.byr = byr;
    }

    public Double getAzn() {
        return azn;
    }

    public void setAzn(Double azn) {
        this.azn = azn;
    }

    public Double getLkr() {
        return lkr;
    }

    public void setLkr(Double lkr) {
        this.lkr = lkr;
    }

    public Double getSdg() {
        return sdg;
    }

    public void setSdg(Double sdg) {
        this.sdg = sdg;
    }

    public Double getSyp() {
        return syp;
    }

    public void setSyp(Double syp) {
        this.syp = syp;
    }

    public Double getMmk() {
        return mmk;
    }

    public void setMmk(Double mmk) {
        this.mmk = mmk;
    }

    public Double getDop() {
        return dop;
    }

    public void setDop(Double dop) {
        this.dop = dop;
    }

    public Double getUzs() {
        return uzs;
    }

    public void setUzs(Double uzs) {
        this.uzs = uzs;
    }

    public Double getKes() {
        return kes;
    }

    public void setKes(Double kes) {
        this.kes = kes;
    }

    public Double getGtq() {
        return gtq;
    }

    public void setGtq(Double gtq) {
        this.gtq = gtq;
    }

    public Double getUry() {
        return ury;
    }

    public void setUry(Double ury) {
        this.ury = ury;
    }

    public Double getHrv() {
        return hrv;
    }

    public void setHrv(Double hrv) {
        this.hrv = hrv;
    }

    public Double getMop() {
        return mop;
    }

    public void setMop(Double mop) {
        this.mop = mop;
    }

    public Double getEtb() {
        return etb;
    }

    public void setEtb(Double etb) {
        this.etb = etb;
    }

    public Double getCrc() {
        return crc;
    }

    public void setCrc(Double crc) {
        this.crc = crc;
    }

    public Double getTzs() {
        return tzs;
    }

    public void setTzs(Double tzs) {
        this.tzs = tzs;
    }

    public Double getTmt() {
        return tmt;
    }

    public void setTmt(Double tmt) {
        this.tmt = tmt;
    }

    public Double getTnd() {
        return tnd;
    }

    public void setTnd(Double tnd) {
        this.tnd = tnd;
    }

    public Double getPab() {
        return pab;
    }

    public void setPab(Double pab) {
        this.pab = pab;
    }

    public Double getLbp() {
        return lbp;
    }

    public void setLbp(Double lbp) {
        this.lbp = lbp;
    }

    public Double getRsd() {
        return rsd;
    }

    public void setRsd(Double rsd) {
        this.rsd = rsd;
    }

    public Double getLyd() {
        return lyd;
    }

    public void setLyd(Double lyd) {
        this.lyd = lyd;
    }

    public Double getGhs() {
        return ghs;
    }

    public void setGhs(Double ghs) {
        this.ghs = ghs;
    }

    public Double getYer() {
        return yer;
    }

    public void setYer(Double yer) {
        this.yer = yer;
    }

    public Double getBob() {
        return bob;
    }

    public void setBob(Double bob) {
        this.bob = bob;
    }

    public Double getBhd() {
        return bhd;
    }

    public void setBhd(Double bhd) {
        this.bhd = bhd;
    }

    public Double getCdf() {
        return cdf;
    }

    public void setCdf(Double cdf) {
        this.cdf = cdf;
    }

    public Double getPyg() {
        return pyg;
    }

    public void setPyg(Double pyg) {
        this.pyg = pyg;
    }

    public Double getUgx() {
        return ugx;
    }

    public void setUgx(Double ugx) {
        this.ugx = ugx;
    }

    public Double getSvc() {
        return svc;
    }

    public void setSvc(Double svc) {
        this.svc = svc;
    }

    public Double getTtd() {
        return ttd;
    }

    public void setTtd(Double ttd) {
        this.ttd = ttd;
    }

    public Double getAfn() {
        return afn;
    }

    public void setAfn(Double afn) {
        this.afn = afn;
    }

    public Double getNpr() {
        return npr;
    }

    public void setNpr(Double npr) {
        this.npr = npr;
    }

    public Double getHnl() {
        return hnl;
    }

    public void setHnl(Double hnl) {
        this.hnl = hnl;
    }

    public Double getBih() {
        return bih;
    }

    public void setBih(Double bih) {
        this.bih = bih;
    }

    public Double getBnd() {
        return bnd;
    }

    public void setBnd(Double bnd) {
        this.bnd = bnd;
    }

    public Double getIsk() {
        return isk;
    }

    public void setIsk(Double isk) {
        this.isk = isk;
    }

    public Double getKhr() {
        return khr;
    }

    public void setKhr(Double khr) {
        this.khr = khr;
    }

    public Double getGel() {
        return gel;
    }

    public void setGel(Double gel) {
        this.gel = gel;
    }

    public Double getMzn() {
        return mzn;
    }

    public void setMzn(Double mzn) {
        this.mzn = mzn;
    }

    public Double getBwp() {
        return bwp;
    }

    public void setBwp(Double bwp) {
        this.bwp = bwp;
    }

    public Double getPgk() {
        return pgk;
    }

    public void setPgk(Double pgk) {
        this.pgk = pgk;
    }

    public Double getJmd() {
        return jmd;
    }

    public void setJmd(Double jmd) {
        this.jmd = jmd;
    }

    public Double getXaf() {
        return xaf;
    }

    public void setXaf(Double xaf) {
        this.xaf = xaf;
    }

    public Double getNad() {
        return nad;
    }

    public void setNad(Double nad) {
        this.nad = nad;
    }

    public Double getAll() {
        return all;
    }

    public void setAll(Double all) {
        this.all = all;
    }

    public Double getSsp() {
        return ssp;
    }

    public void setSsp(Double ssp) {
        this.ssp = ssp;
    }

    public Double getMur() {
        return mur;
    }

    public void setMur(Double mur) {
        this.mur = mur;
    }

    public Double getMnt() {
        return mnt;
    }

    public void setMnt(Double mnt) {
        this.mnt = mnt;
    }

    public Double getNio() {
        return nio;
    }

    public void setNio(Double nio) {
        this.nio = nio;
    }

    public Double getLak() {
        return lak;
    }

    public void setLak(Double lak) {
        this.lak = lak;
    }

    public Double getMkd() {
        return mkd;
    }

    public void setMkd(Double mkd) {
        this.mkd = mkd;
    }

    public Double getAmd() {
        return amd;
    }

    public void setAmd(Double amd) {
        this.amd = amd;
    }

    public Double getMga() {
        return mga;
    }

    public void setMga(Double mga) {
        this.mga = mga;
    }

    public Double getXpf() {
        return xpf;
    }

    public void setXpf(Double xpf) {
        this.xpf = xpf;
    }

    public Double getTjs() {
        return tjs;
    }

    public void setTjs(Double tjs) {
        this.tjs = tjs;
    }

    public Double getHtg() {
        return htg;
    }

    public void setHtg(Double htg) {
        this.htg = htg;
    }

    public Double getBsd() {
        return bsd;
    }

    public void setBsd(Double bsd) {
        this.bsd = bsd;
    }

    public Double getMdl() {
        return mdl;
    }

    public void setMdl(Double mdl) {
        this.mdl = mdl;
    }

    public Double getRwf() {
        return rwf;
    }

    public void setRwf(Double rwf) {
        this.rwf = rwf;
    }

    public Double getKgs() {
        return kgs;
    }

    public void setKgs(Double kgs) {
        this.kgs = kgs;
    }

    public Double getGnf() {
        return gnf;
    }

    public void setGnf(Double gnf) {
        this.gnf = gnf;
    }

    public Double getSrd() {
        return srd;
    }

    public void setSrd(Double srd) {
        this.srd = srd;
    }

    public Double getSll() {
        return sll;
    }

    public void setSll(Double sll) {
        this.sll = sll;
    }

    public Double getXof() {
        return xof;
    }

    public void setXof(Double xof) {
        this.xof = xof;
    }

    public Double getMwk() {
        return mwk;
    }

    public void setMwk(Double mwk) {
        this.mwk = mwk;
    }

    public Double getFjd() {
        return fjd;
    }

    public void setFjd(Double fjd) {
        this.fjd = fjd;
    }

    public Double getErn() {
        return ern;
    }

    public void setErn(Double ern) {
        this.ern = ern;
    }

    public Double getSzl() {
        return szl;
    }

    public void setSzl(Double szl) {
        this.szl = szl;
    }

    public Double getGyd() {
        return gyd;
    }

    public void setGyd(Double gyd) {
        this.gyd = gyd;
    }

    public Double getBif() {
        return bif;
    }

    public void setBif(Double bif) {
        this.bif = bif;
    }

    public Double getKyd() {
        return kyd;
    }

    public void setKyd(Double kyd) {
        this.kyd = kyd;
    }

    public Double getMvr() {
        return mvr;
    }

    public void setMvr(Double mvr) {
        this.mvr = mvr;
    }

    public Double getLsl() {
        return lsl;
    }

    public void setLsl(Double lsl) {
        this.lsl = lsl;
    }

    public Double getLrd() {
        return lrd;
    }

    public void setLrd(Double lrd) {
        this.lrd = lrd;
    }

    public Double getCve() {
        return cve;
    }

    public void setCve(Double cve) {
        this.cve = cve;
    }

    public Double getDjf() {
        return djf;
    }

    public void setDjf(Double djf) {
        this.djf = djf;
    }

    public Double getScr() {
        return scr;
    }

    public void setScr(Double scr) {
        this.scr = scr;
    }

    public Double getSos() {
        return sos;
    }

    public void setSos(Double sos) {
        this.sos = sos;
    }

    public Double getGmd() {
        return gmd;
    }

    public void setGmd(Double gmd) {
        this.gmd = gmd;
    }

    public Double getKmf() {
        return kmf;
    }

    public void setKmf(Double kmf) {
        this.kmf = kmf;
    }

    public Double getStd() {
        return std;
    }

    public void setStd(Double std) {
        this.std = std;
    }

    public Double getBtc() {
        return btc;
    }

    public void setBtc(Double btc) {
        this.btc = btc;
    }

    public Double getXrp() {
        return xrp;
    }

    public void setXrp(Double xrp) {
        this.xrp = xrp;
    }

    public Double getAud() {
        return aud;
    }

    public void setAud(Double aud) {
        this.aud = aud;
    }

    public Double getBgn() {
        return bgn;
    }

    public void setBgn(Double bgn) {
        this.bgn = bgn;
    }

    public Double getJod() {
        return jod;
    }

    public void setJod(Double jod) {
        this.jod = jod;
    }

    public Double getGbp() {
        return gbp;
    }

    public void setGbp(Double gbp) {
        this.gbp = gbp;
    }

    public Double getEth() {
        return eth;
    }

    public void setEth(Double eth) {
        this.eth = eth;
    }

    public Double getEur() {
        return eur;
    }

    public void setEur(Double eur) {
        this.eur = eur;
    }

    public Double getLtc() {
        return ltc;
    }

    public void setLtc(Double ltc) {
        this.ltc = ltc;
    }

    public Double getNzd() {
        return nzd;
    }

    public void setNzd(Double nzd) {
        this.nzd = nzd;
    }

}
